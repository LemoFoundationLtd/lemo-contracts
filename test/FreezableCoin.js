const coinConfig = require('../configs/FreezableCoin')
const testHelper = require('../libs/test_helper')

// Specifically request an abstraction for FreezableCoin
const FreezableCoin = artifacts.require('FreezableCoin')

contract('FreezableCoin', function(accounts) {
    ...
    let instance
    it('deploy', async () => {
        instance = await FreezableCoin.deployed()
    })
    ...
    it('name', async () => {
        ...
        assert.equal(name, 'Coin1')
    })

    it('symbol', async () => {
        const symbol = await instance.symbol()
        assert.equal(symbol, 'COIN1')
    })
    ...
    it('owner.setFreezing(owner, future, 10, 0);freezingBalanceNumberOf(owner);freezingBalanceInfoOf(owner, i)', async () => {
        ...
        assert.equal(event.args.num_coins.toNumber(), amount)
...
async function transfer(fromAccount, toAccount, amount) {
    const instance = await FreezableCoin.deployed()
    const fromAccount_starting_balance = await instance.balanceOf(fromAccount)
...
async function transferAndFreezing(fromAccount, toAccount, amount, freezeAmount, expireTime, freezingType) {
    ...
    assert.equal(setFreezingEvent.args.num_coins.toNumber(), freezeAmount)
